...
/*If the player is alive and has more than 0 arrows
then he would have arrows so making arrowStatus true.
Now the player is allowed to shoot by having more than 0 arrows*/
...
{   if(livePlayer){
        if(arrows > 0)
        {
            ...
            arrowStatus = false;    //If they do not have more than one arrow and try to shoot arrowStatus becomes false
        }                           //So that they can't shoot
    }
}
...
void Player::movePlayer(char** graph, char* dir)
{
      if(strcmp(dir, "left")==0){
            playerDir = "left";
            if(T->GetTicks()>10){
                if(plyLoc.x>-0.9+unitWidth/2){
                    plyLoc.x -= t+0.083;
                    //update plyloc on matrix
                    graph[getPlayerLoc().x+1][getPlayerLoc().y] = '0';
                    graph[getPlayerLoc().x][getPlayerLoc().y] = 'p';
                }
                if(xmax>=1){
                    xmax =1/(float)frames;
                    xmin =0;
   ...
   else if(strcmp(dir, "up")==0)
   {
        playerDir = "up";
            if(T->GetTicks()>1){
                if(plyLoc.y< 0.9-unitWidth/2){
                    plyLoc.y += t+0.083;
                    //update plyloc on matrix
                    graph[getPlayerLoc().x][getPlayerLoc().y-1] = '0';
                    graph[getPlayerLoc().x][getPlayerLoc().y] = 'p';
                }
                if(xmax>=1){
                    xmax =1/(float)frames;
